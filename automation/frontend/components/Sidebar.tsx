...
import { Bot, LayoutDashboard, Github, BookOpen, Settings, LogOut, GitBranch } from "lucide-react";
    ...
    <aside className="w-64 bg-slate-900 min-h-screen flex flex-col">
      ...
      <div className="p-6 border-b border-slate-700">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-white font-semibold text-sm">AI Orchestrator</p>
            <p className="text-slate-400 text-xs">ML Automation</p>
          </div>
        </div>
      </div>
          ...
          return (
            <Link key={item.href} href={item.href}
              className={clsx(
                ...
                active ? "bg-sky-600 text-white" : "text-slate-400 hover:text-white hover:bg-slate-800"
              )}>
